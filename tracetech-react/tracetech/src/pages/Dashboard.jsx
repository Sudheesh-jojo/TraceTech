...
import { getTodayForecast, getImpactSummary, getWeather, getDailyWaste } from '../api/api';
...
export default function Dashboard({ onNavigate }) {
  ...
  const filtered = tab === 'All'
    ? forecasts
    : forecasts.filter((f) => f.stallId === STALL_MAP[tab]);
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="card" style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                {wd.items.length > 0 && (
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {wd.items.map((item) => (
                      <span key={item} className="weather-item-tag">{item}</span>
                    ))}
                  </div>
                )}
              </div>
              <div style={{ textAlign: 'right' }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{wd.icon}</div>
              </div>
            </div>
          </div>
          ...
          <div className="charts-row">
            <div className="card">
              <div className="card-title">Waste trend -- this month (Daily)</div>
              <LineChartWidget
                labels={chartLabels}
                datasets={[
                  {
                ...
                <tbody>
                  {filtered.map((f) => (
                    <tr key={f.itemId}>
                      ...
                      <td className="fs12" style={{ color: 'var(--text-muted)' }}>
                        {['', 'Snacks', 'Juices', 'South Indian', 'Lunch'][f.stallId]}
                      </td>
